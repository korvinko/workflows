...
export async function createInvoice(browser: puppeteer.Browser): Promise<boolean> {
	...
	await page.select('#sales_invoice_proforma_form_currency', process.env.TS_INVOICE_TRANSFER_CURRENCY);
	await page.type('#sales_invoice_proforma_form_payment_exchange', process.env.TS_INVOICE_TRANSFER_CURRENCY_RATE);
	await page.select('#sales_invoice_proforma_form_bankAccount', process.env.TS_INVOICE_TRANSFER_CURRENCY_BANK_ACCOUNT);
